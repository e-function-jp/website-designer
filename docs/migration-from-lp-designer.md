...
muuuuu.org の **タクソノミアーカイブを巡回**して収集する。

当初は新着一覧を取り、タイトルと URL のキーワードでサイト種別を推測していたが、
...
タクソノミを持つ。掲載側が人手で付けた分類なので、こちらの推測より正確で安定している。
そこで **アーカイブに載っているという事実**をもって分類する方式へ変えた。

| | 旧（キーワード推測） | 新（タクソノミ巡回） |
|---|---|---|
| 収集数 | 190 件 | 981 件 |
| サイト種別 未分類 | 98 件（52%） | **0 件（0%）** |
| 業種 未分類 | 97 件（51%） | 48 件（5%） |

- サイト種別は `GALLERY_SITETYPE_MAP`（muuuuu の sitetype → 本プロジェクトの id）で引く。
  複数該当は `CLASSIFY_ORDER` で決める（`co,recruit` → `recruit`。具体的な種別が優先）
- 業種は muuuuu の 34 種をそのまま採用。自前で 8 種に丸めていたのをやめた
  （業種軸は参照を揃えるのに使うだけで品質ルールには効かないので、
  収集元の分類をそのまま持つのが正しい）
- 日本市場向けサイトを作るのが目的なので、`overseas-site` アーカイブを
  除外集合として取得し海外サイトを外す（実測 72 件）
- キーワード分類は、タクソノミを持たない収集元のためのフォールバックとして残す

> 業種は当初、各アイテムの詳細ページ URL（`/industry/{slug}/{id}.html`）から
> 拾おうとしたが、**パーマリンクは industry とは限らない**（`/color/white/{id}.html`
> のこともある）。981 件中 173 件がこれで取れず、業種も archive 巡回に統一した。
